...
static void
assert_issuer_is_ca (GTlsCertificate *certificate)
{
  ...
  ca = g_tls_certificate_new_from_file (tls_test_file_path ("ca.pem"), NULL);
  g_assert_nonnull (ca);

  g_assert_true (g_tls_certificate_is_same (issuer, ca));
...
{
  GTlsCertificate *cert;
  GByteArray *pkcs12_data, *pkcs12_enc_data;
  GError *error = NULL;
...
static void
test_certificate_pkcs12_chain (void)
{
  GTlsCertificate *cert;
  GByteArray *pkcs12_data;
  GError *error = NULL;

  pkcs12_data = load_bytes_for_test_file ("client-and-key-fullchain.p12");
  cert = g_tls_certificate_new_from_pkcs12 (pkcs12_data->data, pkcs12_data->len, NULL, &error);
  g_assert_no_error (error);
  g_assert_nonnull (cert);
  assert_cert_contains_cert_and_key (cert);
  assert_equals_original_cert (cert);
  assert_issuer_is_ca (cert);
      ...
      char *argv[])
{
  ...
  g_test_add_func ("/tls/" BACKEND "/certificate/pkcs12/chain", test_certificate_pkcs12_chain);
